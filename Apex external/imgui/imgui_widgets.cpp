...
void ImGui::SeparatorText(const char* label)
{
    ...
    // Get the current context and style
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;

    // Calculate text size and positions
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos + ImVec2(0, style.FramePadding.y);

    // Define the separator height and width
    const float separatorHeight = 2.0f; // Height of the separator line
    const float separatorWidth = CalcItemWidth();

    // Define rectangles for the separator
    ImRect separator_bb(pos, pos + ImVec2(separatorWidth, separatorHeight));

    // Define colors
    ImColor glowColor = ImColor(255, 100, 255, 200); // Bright glow color
    ImColor baseColor = ImColor(40, 10, 60, 255);    // Base color for the separator
    ImColor fadeColor = ImColor(255, 255, 255, 0);   // Transparent color for fading effect

    // Draw the glowing effect
    const float glowThickness = 20.0f; // Thickness of the glow effect
    for (float i = 0; i < glowThickness; i += 2.0f)
    {
        float alpha = ImSaturate((glowThickness - i) / glowThickness);
        ImColor currentGlowColor = ImColor(glowColor.Value.x, glowColor.Value.y, glowColor.Value.z, alpha * 255);
        window->DrawList->AddRectFilled(separator_bb.Min - ImVec2(i, 0), separator_bb.Max + ImVec2(i, 0), currentGlowColor, 0, ImDrawFlags_RoundCornersNone);
    }

    // Draw the separator line
    window->DrawList->AddRectFilled(separator_bb.Min, separator_bb.Max, baseColor);

    // Render the label
    RenderText(ImVec2(pos.x + (separatorWidth - label_size.x) * 0.5f, pos.y - label_size.y), label);

    // Update cursor position
    window->DC.CursorPos.y += label_size.y + style.ItemSpacing.y;
}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float sliderHeight = label_size.y + style.FramePadding.y * 1.1f;  // Reduced height for a thinner appearance
    const ImVec2 sliderSize(ImGui::CalcItemWidth(), sliderHeight);

    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + sliderSize);
    ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0 ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    bool hovered, held;
    bool value_changed = ImGui::ButtonBehavior(frame_bb, id, &hovered, &held);

    // Calculate normalized value for slider
    float t = (*v - v_min) / (v_max - v_min);
    t = ImSaturate(t);

    // Define colors
    auto borderColor = ImColor(50, 50, 70, 255);            // Dark gray border
    auto baseColor = ImColor(30, 30, 30, 255);               // Dark background
    auto baseColorHovered = ImColor(60, 60, 60, 255);        // Lighter gray on hover
    auto sliderFillColor = ImColor(150, 0, 255, 255);        // Strong purple for slider fill

    // Draw slider background
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, hovered ? baseColorHovered : baseColor, 4.0f);

    // Draw filled slider portion
    ImVec2 fillEnd = ImVec2(frame_bb.Min.x + t * frame_bb.GetWidth(), frame_bb.Max.y);
    window->DrawList->AddRectFilled(frame_bb.Min, fillEnd, sliderFillColor, 4.0f);

    // Draw border
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, borderColor, 4.0f, 0, 1.0f);

    // Handle input
    if (held)
    {
        const float mouse_pos = g.IO.MousePos.x - frame_bb.Min.x;
        t = mouse_pos / (frame_bb.Max.x - frame_bb.Min.x);
        t = ImSaturate(t);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Slider dimensions
    const float sliderHeight = 8.0f;  // Height of the slider track
    const float handleWidth = 20.0f;   // Width of the handle
    ...
    float t = (*v - v_min) / (float)(v_max - v_min);  // Current value as a percentage

    // Update value on press/drag
    if (pressed || held)
    {
        const float mouse_pos = GetMousePos().x - frame_bb.Min.x; // Get current mouse position relative to the slider
        t = ImClamp(mouse_pos / frame_bb.GetWidth(), 0.0f, 1.0f); // Clamp value to [0, 1]
        *v = static_cast<int>(ImLerp(static_cast<float>(v_min), static_cast<float>(v_max), t)); // Calculate new value
    }

    // Define colors with a solid dark purple theme
    ImColor trackColor = ImColor(30, 30, 50, 255);        // Darker track color
    ImColor fillColor = ImColor(90, 30, 140, 255);        // Brighter filled section
    ImColor handleColor = ImColor(80, 20, 120, 255);      // Darker handle color

    // Draw the slider track (rectangle)
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, trackColor, sliderHeight * 0.5f);

    // Draw the filled section (rectangle with progress)
